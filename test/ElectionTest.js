...
contract('Election', function(accounts) {
    ...
    let partyNames = [
      ...
      "0x7369656d6b620000000000000000000000000000000000000000000000000000",
      "0x7369656d6b630000000000000000000000000000000000000000000000000000"
    ];
    ...
    describe('Deploying contract', function () {
        it('should test that the smart contract is deployed', async function () {
            assert.equal(web3.utils.isAddress(ElectionInst.address), true);
        })
    })
    ...
    describe('vote',function(){
        it('Can\'t vote on yourself', async function(){
            try {
                await ElectionInst.vote(1, {from : voterAddrs[1]})
                assert.fail();
            } catch(err){
                        console.log("HERE COMES THE ERROR:", err.toString());
                        assert.ok(/revert/.test(err.message));
        ...
        it('vote count should increase', async function(){
            let prevVoteCnt = await ElectionInst.getVoteCount(1);
            console.log('prev voter cnt = ' + prevVoteCnt);
            await ElectionInst.giveRightToVote(accounts[1], { from: accounts[0] });
            console.log("voter weight",await ElectionInst.getVoterWeight(accounts[1]));
            await ElectionInst.vote(1, {from : voterAddrs[0]})
            console.log("voter weight",await ElectionInst.getVoterWeight(accounts[1]));
            let aftVoteCnt = await ElectionInst.getVoteCount(1);
